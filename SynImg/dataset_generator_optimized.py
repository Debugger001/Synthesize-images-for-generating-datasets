...
def create_image_anno(objects, distractor_objects, img_file, semantic_file, instance_file, anno_file, blur, w=WIDTH, h=HEIGHT, scale_augment=False, rotation_augment=False, blending_list=['none'], dontocclude=True):
    ...
    generated_files = glob.glob(os.path.join('/home/lc/syndata-generation/output_dir_10_23_19/train/', '*.jpg'))
    ...
    while True:
        ...
        backgrounds.append(Image.fromarray(np.zeros((512, 512, 3), dtype=np.uint8)))
        ...
        dir_list = tuple(open('/home/lc/syndata-generation/data_dir/objects_dir_new_with_front/dir_list.txt', 'r'))
        ...
        for idx, obj in enumerate(all_objects):
            ...
            if obj[1] in gray_value_dict:
                ...
                gray_value_array = np.zeros((sem_mask_np.shape[0], sem_mask_np.shape[1]), dtype = np.uint8)
                gray_value_array.fill(gray_value_dict[obj[1]])
                instance_pixel_array = sem_mask_np > 0
                sem_mask = instance_pixel_array * gray_value_array
                ...
                gray_value_array = np.zeros((ins_mask_np.shape[0], ins_mask_np.shape[1]), dtype=np.uint8)
                gray_value_array.fill(gray_value_offset)
                instance_pixel_array = ins_mask_np > 0
                ins_mask = instance_pixel_array * gray_value_array
                ...
                sem_mask = np.zeros((sem_mask_np.shape[0], sem_mask_np.shape[1]), dtype=np.uint8)
                gray_value_array = np.zeros((sem_mask_np.shape[0], sem_mask_np.shape[1]), dtype=np.uint8)
                gray_value_array.fill(255)
                dinstance_pixel_array = sem_mask_np > 0
                sem_mask = dinstance_pixel_array * gray_value_array
                # for sem_mask_rows in range(0, sem_mask_np.shape[0]):
                #     for sem_mask_cols in range(0, sem_mask_np.shape[1]):
                #         if sem_mask_np[sem_mask_rows][sem_mask_cols] > 0:
                #             sem_mask[sem_mask_rows][sem_mask_cols] = 255
                sem_mask_PIL = Image.fromarray(sem_mask)
                ...
                gray_value_array = np.zeros((ins_mask_np.shape[0], ins_mask_np.shape[1]), dtype=np.uint8)
                gray_value_array.fill(255)
                dinstance_pixel_array = ins_mask_np > 0
                ins_mask = dinstance_pixel_array * gray_value_array
                # for ins_mask_rows in range(0, ins_mask_np.shape[0]):
                #     for ins_mask_cols in range(0, ins_mask_np.shape[1]):
                #         if ins_mask_np[ins_mask_rows][ins_mask_cols] > 0:
                #                 ins_mask[ins_mask_rows][ins_mask_cols] = 255
            ...
            # for i in range(len(blending_list)):
            if blur == 'none' or blur == 'motion':

                backgrounds[0].paste(foreground, (x, y), mask)

                # if obj[1] in gray_value_dict:
                    # sem_backgrounds[i][obj_num].paste(sem_mask_PIL, (x, y), mask)
                    # ins_backgrounds[i][obj_num].paste(ins_mask_PIL, (x, y), mask)
                sem_backgrounds[0].paste(sem_mask_PIL, (x, y), mask)
                ins_backgrounds[0].paste(ins_mask_PIL, (x, y), mask)
                ...
                img_target = PIL2array3C(backgrounds[0])
                ...
                backgrounds[0] = Image.fromarray(background_array, 'RGB')
                # if obj[1] in gray_value_dict:
                sem_backgrounds[0].paste(sem_mask_PIL, (x, y), mask)
                ins_backgrounds[0].paste(ins_mask_PIL, (x, y), mask)
                ...
                backgrounds[0].paste(foreground, (x, y), Gaussian_mask)
                Gaussian_mask_np = np.array(Gaussian_mask.getdata(), np.uint8).reshape(Gaussian_mask.size[1], Gaussian_mask.size[0])
                augment_gv_array = np.zeros((Gaussian_mask.size[1], Gaussian_mask.size[0]), dtype=np.uint8)
                augment_gv_array.fill(255)
                augment_pos_array = Gaussian_mask_np >= 100
                Gaussian_mask_np = augment_gv_array * augment_pos_array
                ...
                sem_backgrounds[0].paste(sem_mask_PIL, (x, y), mask_Gaussian_mask)
                ins_backgrounds[0].paste(ins_mask_PIL, (x, y), mask_Gaussian_mask)

            elif blur == 'box':
                Box_mask = Image.fromarray(cv2.blur(PIL2array1C(mask),(3,3)))

                backgrounds[0].paste(foreground, (x, y), Box_mask)
                Box_mask_np = np.array(Box_mask.getdata(), np.uint8).reshape(Box_mask.size[1], Box_mask.size[0])
                augment_gv_array = np.zeros((Box_mask.size[1], Box_mask.size[0]), dtype=np.uint8)
                augment_gv_array.fill(255)
                augment_pos_array = Box_mask_np >= 100
                Box_mask_np = augment_gv_array * augment_pos_array
                ...
                # if obj[1] in gray_value_dict:
                sem_backgrounds[0].paste(sem_mask_PIL, (x, y), mask_Box_mask)
                ins_backgrounds[0].paste(ins_mask_PIL, (x, y), mask_Box_mask)
    ...
    if blur == 'motion':
        backgrounds[0] = LinearMotionBlur3C(PIL2array3C(backgrounds[0]))
#
#     # print("???")
    backgrounds[0].save(img_file)
#
    # print(semantic_file)
    sem_backgrounds[0].save(semantic_file)
#     # print("%i pixel values in the image"%count_pixel_value(sem_backgrounds[i]))
#     # print("%i objects in the image"%len(objects))
#
    ins_backgrounds[0].save(instance_file)
...
def gen_syn_data(image_files, labels, img_dir, anno_dir, sem_mask_dir, ins_mask_dir, scale_augment, rotation_augment, dontocclude, add_distractors):
    ...
    for i in tqdm(range(12000)):
        for blur in BLENDING_LIST:
            ...
            params = (objects, distractor_objects, img_file, semantic_file, instance_file, anno_file, blur)
...
def parse_args():
    ...
    parser.add_argument("--root", default = "/home/lc/syndata-generation/data_dir/objects_dir_new_with_front",
      help="The root directory which contains the images and annotations.")
    parser.add_argument("--exp", default = "/home/lc/syndata-generation/output_dir_10_23_19",
