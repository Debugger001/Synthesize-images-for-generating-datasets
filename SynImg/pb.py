...
def create_mask(img_mask, img_target, img_src, offset=(0, 0)):
    ...
    ht = img_target.shape[0]
    wt = img_target.shape[1]
...
def get_gradient_sum_1C(img, i, j, h, w):
    """
    Return the sum of the gradient of the source imgae.
    * 1D array for RGB
    """
...
def get_mixed_gradient_sum_1C(img_src, img_target, i, j, h, w, ofs,
                           c=1.0):
    """
    Return the sum of the gradient of the source imgae.
    * 1D array for RGB
    ...
    for kk in range(4):
        ...
        v_sum += np.array([fp if abs(fp * c) > abs(gp) else gp])
...
def poisson_blend_1C(img_mask, img_src, img_target, method='mix', c=1.0,
                      offset_adj=(0,0)):

        hm, wm = img_mask.shape
        region_size = hm * wm

        F = np.zeros((region_size))
        A = scipy.sparse.identity(region_size, format='lil')
        ...
        # poisson blending
            ...
            if method == 'mix':
                grad_func = lambda ii, jj: get_mixed_gradient_sum_1C(
                    img_src, img_target, ii, jj, hm, wm, offset_adj, c=c)
            else:
                grad_func = lambda ii, jj: get_gradient_sum_1C(
                    img_src, ii, jj, hm, wm)
        ...
        # x = pyamg.solve(A, F[:, l], verb=True, tol=1e-15, maxiter=100)
        x = scipy.sparse.linalg.spsolve(A, F[:])
        x[x > 255] = 255
        x[x < 0] = 0
        x = np.array(x, img_pro.dtype)

        img_pro[offset_adj[0]:offset_adj[0] + hm,
                offset_adj[1]:offset_adj[1] + wm]\
            = x.reshape(hm, wm, order='F')
